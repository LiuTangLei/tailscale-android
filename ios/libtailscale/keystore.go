...
func (k *hardwareAttestationKey) fetchPublic() error {
	...
	if err != nil {
		return fmt.Errorf("loading public key for id %q: %w", k.id, err)
	}
...
func (k *hardwareAttestationKey) Sign(rand io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	if k.id == "" || k.appCtx == nil {
...
func (k *hardwareAttestationKey) UnmarshalJSON(data []byte) error {
	...
	if err := k.appCtx.HardwareAttestationKeyLoad(k.id); err != nil {
		return fmt.Errorf("loading key with ID %q: %w", k.id, err)
	}
